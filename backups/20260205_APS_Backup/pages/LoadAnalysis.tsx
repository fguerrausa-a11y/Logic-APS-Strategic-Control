      ...
      if (wcs && ops && scenario && allMachines) {
        ...
        if (loads.length > 0) {
          ...
          setTocStatus({
            drum: drumWc.name,
            buffer: bufferHours,
            rope: drumWc.load > 90 ? 'High Tension' : 'Pulling'
          });
        }
  ...
  const fetchBuffers = async () => {
    ...
    try {
      const { data: items } = await supabase
        .from('items')
        .select('*')
        .filter('item_type', 'in', '("COMPRADO","MP","INSUMO","PURCHASED")')
        .limit(10);
      ...
      if (items) {
        setBuffers(items.map(item => {
          ...
          // SCALING: Use 3 * safety as the "100%" mark for the bar to show more variance
          ...
          <div className="h-full max-w-[1600px] mx-auto flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 shrink-0">
              ...
              <div className="bg-amber-600/10 border border-amber-500/20 p-4 rounded-[1.5rem] flex items-center justify-between">
                <div>
                  <p className="text-[9px] font-black uppercase text-amber-500 tracking-widest mb-0.5">BUFFER DE TIEMPO</p>
                  <h4 className="text-base font-black uppercase tracking-tighter">{tocStatus.buffer}h de Reserva</h4>
                </div>
                <Clock className="text-amber-500" size={24} />
              </div>
              <div className="bg-emerald-600/10 border border-emerald-500/20 p-4 rounded-[1.5rem] flex items-center justify-between">
                <div>
                  <p className="text-[9px] font-black uppercase text-emerald-500 tracking-widest mb-0.5">THE ROPE (FLUJO)</p>
              ...
              <div className="xl:col-span-2 rounded-[2rem] border border-[var(--border-color)] bg-[var(--bg-card)] p-6 flex flex-col shadow-xl min-h-0 relative overflow-hidden">
                <div className="flex items-center justify-between mb-4 shrink-0">
                  <div>
                    ...
                    <p className="text-[var(--text-muted)] text-[10px] font-bold uppercase tracking-widest opacity-60">Capacidad Finita vs Carga Proyectada</p>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-6">
                        ...
                        <div className="flex justify-between items-start gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              <span className="text-[8px] font-bold text-[var(--text-muted)] uppercase tracking-widest">STOCK ACTUAL:</span>
                          ...
                          <div className="text-right shrink-0">
                            ...
                            <div className="text-[7px] font-bold text-[var(--text-muted)] uppercase tracking-widest mt-1">META: {buf.safety * 2}</div>
                          </div>
                        </div>
                    ...
                    {buffers.length === 0 && (
                      <div className="flex flex-col items-center justify-center py-10 opacity-20 text-center">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest">Sin datos de inventario</p>
                      </div>
                    )}
                  </div>
                </div>
