...
const ShiftSettings: React.FC = () => {
    ...
    return (
        <div className="space-y-4 bg-surface-dark p-6 rounded-[2rem] border border-surface-border">
            ...
            <div className="grid gap-4">
                {shifts.map(s => (
                    <div key={s.id} className="p-4 bg-background-dark/50 rounded-2xl border border-surface-border space-y-4">
                        {editingShift?.id === s.id ? (
                            <div className="space-y-4">
                                <input
                                    className="w-full bg-background-dark border border-surface-border rounded-xl p-2 text-sm font-bold shadow-inner"
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase font-black text-[#9dabb9]">{t('start')}</label>
                                        <input type="time" className="w-full bg-background-dark border border-surface-border rounded-xl p-2 text-xs font-bold" value={editingShift.start_time} onChange={e => setEditingShift({ ...editingShift, start_time: e.target.value })} />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[10px] uppercase font-black text-[#9dabb9]">{t('end')}</label>
                                        <input type="time" className="w-full bg-background-dark border border-surface-border rounded-xl p-2 text-xs font-bold" value={editingShift.end_time} onChange={e => setEditingShift({ ...editingShift, end_time: e.target.value })} />
                                    </div>
                                </div>
                                <div className="flex justify-end gap-2 pt-2">
                                    <button onClick={() => setEditingShift(null)} className="text-[10px] font-black uppercase text-[#9dabb9] px-3 py-2">{t('cancel')}</button>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs text-[#9dabb9] tracking-widest uppercase font-black">{s.start_time} - {s.end_time} • {s.days_of_week?.length || 5} {t('days')}</p>
                                </div>
                ...
                {shifts.length === 0 && <p className="text-xs text-[#9dabb9] italic">No hay turnos definidos.</p>}
            </div>
        </div>
    );
};
...
const MachineSettings: React.FC = () => {
    ...
    return (
        <div className="space-y-4">
            {machines.map(m => (
                <div key={m.id} className="flex flex-wrap items-center justify-between p-4 bg-surface-dark rounded-xl border border-surface-border gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-[#9dabb9] font-black uppercase tracking-widest">{m.work_center?.name || 'S/C'}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <select
                            ...
                            className="bg-background-dark border border-surface-border rounded-xl px-3 py-2 text-xs font-bold outline-none focus:border-indigo-500 transition-all min-w-[150px]"
                        >
...
const WorkCenterSettings: React.FC = () => {
    ...
    return (
        <div className="space-y-4">
            {wcs.map(wc => (
                <div key={wc.id} className="flex flex-wrap items-center justify-between p-4 bg-surface-dark rounded-xl border border-surface-border gap-4">
                    <div className="flex items-center gap-4">
                        <div className="size-10 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400">
                            <span className="material-symbols-outlined">precision_manufacturing</span>
                        </div>
                        <div>
                            <p className="font-bold text-white">{wc.name}</p>
                            <p className="text-xs text-[#9dabb9]">ID: {wc.id}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex flex-col gap-1 w-48">
                            <div className="flex justify-between">
                                <span className="text-[10px] text-[#9dabb9] uppercase font-bold">{t('overlap')}</span>
...
const SettingsPage: React.FC = () => {
    ...
    if (loading) return <div className="flex h-screen items-center justify-center bg-background-dark text-white uppercase font-black tracking-widest">{t('loading_params')}</div>;

    return (
        <div className="flex h-screen overflow-hidden bg-background-dark font-body transition-colors">
            <Sidebar />
            <main className="flex-1 flex flex-col h-full overflow-hidden relative text-white">
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-surface-border pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#9dabb9]">{t('engine_desc')}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-[#9dabb9] uppercase tracking-wider">{t('scheduling_modality')}</label>
                                    <select
                                        value={settings.scheduling_mode}
                                        onChange={(e) => setSettings({ ...settings, scheduling_mode: e.target.value })}
                                        className="w-full bg-surface-dark border border-surface-border rounded-xl p-3 text-sm focus:border-indigo-500 outline-none transition-all"
                                    >
                                        <option value="TOC Optimized">TOC Optimized (Theory of Constraints)</option>
                                        <option value="Forward ASAP">Forward ASAP</option>
                                        <option value="Backward JIT">Backward JIT</option>
                                    </select>
                                </div>

                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-[#9dabb9] uppercase tracking-wider">{t('bottleneck_mgmt')}</label>
                                    <select
                                        value={settings.bottleneck_management}
                                        onChange={(e) => setSettings({ ...settings, bottleneck_management: e.target.value })}
                                        className="w-full bg-surface-dark border border-surface-border rounded-xl p-3 text-sm focus:border-indigo-500 outline-none transition-all"
                                    >
                                        <option value="Drum-Buffer-Rope">Drum-Buffer-Rope (TOC Standard)</option>
                                        <option value="Infinite Capacity">{t('finite_capacity').replace('Finita', 'Infinita')}</option>
                                    </select>
                                </div>
                            </div>
                        </section>
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-surface-border pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#9dabb9]">{t('sync_desc')}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="flex items-center justify-between p-4 bg-surface-dark rounded-xl border border-surface-border">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-xs text-[#9dabb9]">{t('overlap_desc')}</p>
                                    </div>
                                    <input
                                        type="checkbox"
                                        checked={settings.overlap_enabled}
                                        onChange={(e) => setSettings({ ...settings, overlap_enabled: e.target.checked })}
                                        className="size-5 rounded border-surface-border text-indigo-500 focus:ring-indigo-500"
                                    />
                                </div>

                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-[#9dabb9] uppercase tracking-wider">{t('default_overlap_pct')}</label>
                                    <div className="flex items-center gap-4">
                                        <input
                                            type="range"
                                            min="0"
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-surface-border pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#9dabb9]">{t('reprovision_desc')}</p>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-[#9dabb9] uppercase tracking-wider">{t('inventory_policy')}</label>
                                    <select
                                        value={settings.reprovision_policy}
                                        onChange={(e) => setSettings({ ...settings, reprovision_policy: e.target.value })}
                                        className="w-full bg-surface-dark border border-surface-border rounded-xl p-3 text-sm focus:border-indigo-500 outline-none transition-all"
                                    >
                                        <option value="TOC Replenishment">TOC Replenishment (Dynamic Buffers)</option>
                                        <option value="MRP Standard">MRP Standard (Push / Explosive)</option>
                                        <option value="Stock-to-Order">Stock-to-Order (Direct Pull)</option>
                                    </select>
                                </div>

                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-[#9dabb9] uppercase tracking-wider">{t('default_buffer_days')}</label>
                                    <input
                                        type="number"
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-surface-border pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#9dabb9]">{t('shift_desc')}</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                                <ShiftSettings />
                                <div className="space-y-6">
                                    <h3 className="text-sm font-black uppercase tracking-widest text-[#9dabb9]">{t('machine_assignment')}</h3>
                                    <MachineSettings />
                                </div>
                            </div>
                        </section>
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-surface-border pb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-[#9dabb9]">{t('wc_desc')}</p>
                                </div>
                            </div>
