...
const SchedulePage: React.FC = () => {
  ...
  const init = async () => {
    try {
      const { data: scens } = await supabase.from('scenarios').select('*').order('created_at', { ascending: false });
      const { data: machinesData } = await supabase.from('machines').select('*, work_center:work_centers(name)');
      setScenarios(scens || []);
      setMachines(machinesData || []);
      if (scens && scens.length > 0) {
        setSelectedScenarioId(scens[0].id);
        loadScenarioData(scens[0].id);
      }
    } catch (e) { console.error(e); }
  };

  const loadScenarioData = async (id: string | null) => {
    if (!id) return;
    setLoading(true);
    try {
      ...
      const finalOpsMapped = (opsToUse || []).map((dbOp: any) => {
        ...
        // Better mapping: same item and same WO reference
      ...
      setOperations(prev => {
        // Prevent duplicates by checking (work_order_id, operation_sequence)
        const unique = new Map();
        [...prev, ...finalOpsMapped].forEach(o => {
          const key = `${o.work_order_id}-${o.operation_sequence}`;
          unique.set(key, o);
        });
        return Array.from(unique.values());
      });

      // --- EXHAUSTIVE MACHINE RESOLUTION (Show all capacity, not just used) ---
      ...
      if (finalOpsMapped.length > 0) {
        ...
        if (opTimes.length > 0) {
          ...
          // Set view range to start 6 hours before the first operation and show 120 units of time
          setViewRange({
  ...
  const handleUnlockAll = async () => {
    ...
    try {
      ...
      await loadScenarioData(selectedScenarioId);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleResetScenario = async () => {
    if (!selectedScenarioId) return;
    if (!window.confirm("¿Estás seguro de que deseas eliminar todos los ajustes manuales y volver al cálculo original?")) return;

    try {
      setLoading(true);
      await supabase.from('proposed_operations').update({ is_locked: false }).eq('scenario_id', selectedScenarioId);
      await loadScenarioData(selectedScenarioId);
      alert(t('manual_adjustments_reset'));
    } catch (e) {
      console.error(e);
      alert(t('error_reset_scenario'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteScenario = async () => {
    if (!selectedScenarioId) return;
    if (!window.confirm("¿Confirmas la ELIMINACIÓN de esta simulación?")) return;
    try {
      setLoading(true);
      await scenarioService.deleteScenario(selectedScenarioId);
      const { data: scens } = await supabase.from('scenarios').select('*').order('created_at', { ascending: false });
      setScenarios(scens || []);
      if (scens && scens.length > 0) {
        setSelectedScenarioId(scens[0].id);
  ...
  const handleOperationDrop = async (op: any, newStartDate: Date, machineId: string) => {
    ...
    try {
      ...
      const res = await apsAlgorithm.runSimulation(newScenario.id);
      const { data: allScens } = await supabase.from('scenarios').select('*').order('created_at', { ascending: false });

      setScenarios(allScens || []);
      setSelectedScenarioId(newScenario.id);
      loadScenarioData(newScenario.id);
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-1 gap-1 shadow-inner">
              {(['hours', 'days', 'weeks'] as const).map(g => (
                <button
                  ...
                  title={t(g)}
                >
                  {g === 'hours' ? t('hours').charAt(0) : g === 'days' ? t('days').charAt(0) : t('weeks').charAt(0)}
                </button>
              ))}
            ...
            <div className="flex items-center gap-1.5 bg-[var(--bg-input)] p-1 rounded-xl border border-[var(--border-color)]">
              <select
                ...
                onChange={(e) => { setSelectedScenarioId(e.target.value); loadScenarioData(e.target.value); }}
                className="bg-transparent px-3 py-1.5 text-xs font-bold outline-none min-w-[220px]"
              >
                {scenarios.map(s => <option key={s.id} value={s.id}>{s.name || 'Escenario'}</option>)}
              </select>
            </div>
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-[var(--bg-main)]/80 backdrop-blur-sm">
                  <div className="text-center p-12 glass-panel rounded-[3rem] border border-indigo-500/20 max-w-sm animate-in zoom-in duration-300">
                    ...
                    <div className="flex flex-col gap-3 mt-8">
                      <button onClick={() => selectedScenarioId && loadScenarioData(selectedScenarioId)} className="bg-indigo-600 hover:bg-indigo-500 text-white px-10 py-4 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center justify-center gap-2">
