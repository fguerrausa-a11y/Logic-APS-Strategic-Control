...
const AIAnalyst: React.FC<AIAnalystProps> = ({ simulationData }) => {
    ...
    return saved ? JSON.parse(saved) : [
      {
        ...
        key: 'ai_welcome_msg',
        timestamp: new Date().toLocaleTimeString(language === 'es' ? 'es-AR' : 'en-US', { hour: '2-digit', minute: '2-digit' })
      }
    ];
  });
  ...
  useEffect(() => {
    // These match the 'es' entries in services/i18n/aiAnalyst.ts
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await sendMessageToGemini(input, simulationData, language);
      ...
      <div className="p-4 border-b border-[var(--border-color)] flex items-center justify-between bg-[var(--bg-card)]">
        <div className="flex items-center gap-3">
          <div className="size-8 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-600/20">
            <span className="material-symbols-outlined text-white text-sm" translate="no">smart_toy</span>
          </div>
          <div>
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`size-8 rounded-full flex items-center justify-center shrink-0 shadow-md ${msg.role === 'assistant'
              ...
              <span className="material-symbols-outlined text-white text-xs" translate="no">
        ...
        {isTyping && (
          <div className="flex gap-3">
            <div className="size-8 rounded-full bg-indigo-600 flex items-center justify-center shrink-0 animate-pulse">
              <span className="material-symbols-outlined text-white text-xs" translate="no">smart_toy</span>
            </div>
