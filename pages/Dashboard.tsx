...
const Dashboard: React.FC = () => {
    ...
    try {
      ...
      if (pwos) {
        ...
        if (realAlerts.length === 0) {
          realAlerts.push({ title: t('alert_new_order'), desc: t('alert_scenario_applied'), time: t('just_now'), type: 'success' });
        }
        ...
        const sortedDays = Object.entries(dayCounts)
          ...
          .slice(0, 7)
          .map(([date, value]) => ({
            label: new Date(date).toLocaleDateString(language === 'es' ? 'es-AR' : 'en-US', { day: 'numeric', month: 'numeric' }),
            value
          }));
  ...
  const kpis = [
    ...
    },
    {
      ...
      unit: t('orders'),
      trend: stats.delays > 5 ? t('high') : t('low'),
