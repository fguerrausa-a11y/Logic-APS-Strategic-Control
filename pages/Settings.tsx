        ...
        <div className="space-y-4 bg-[var(--bg-card)] p-6 rounded-[2rem] border border-[var(--border-color)]">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={addShift} className="text-xs bg-indigo-600 hover:bg-indigo-500 px-4 py-2 rounded-xl font-bold transition-all flex items-center gap-2">
                    <span className="material-symbols-outlined text-sm" translate="no">add</span> {t('new_shift')}
                </button>
            </div>
...
const SettingsPage: React.FC = () => {
    const { t, language, setLanguage, refreshTranslations, isLoading: syncingI18n } = useTranslation();
    const [loading, setLoading] = useState(true);
    ...
    return (
        <div className="flex h-full overflow-hidden bg-[var(--bg-main)] transition-colors">
            <main className="flex-1 flex flex-col h-full overflow-hidden relative text-[var(--text-main)]">
                <TopHeader title={t('settings')} icon="settings" />
                    ...
                    <div className="max-w-[1000px] mx-auto space-y-10">
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4">
                                ...
                                <div>
                                    <h2 className="text-xl font-bold font-display">{t('aps_strategy')}</h2>
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4">
                                <span className="material-symbols-outlined text-slate-400 text-3xl">display_settings</span>
                                <div>
                                    <h2 className="text-xl font-bold font-display">{t('system_prefs')}</h2>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="p-4 bg-indigo-500/5 rounded-2xl border border-indigo-500/10 space-y-3">
                                        <div className="flex items-center gap-3">
                                            <div className={`size-10 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 ${syncingI18n ? 'animate-spin' : ''}`}>
                                                <span className="material-symbols-outlined">sync</span>
                                            </div>
                                            <div className="flex-1">
                                                <h3 className="text-sm font-bold">{t('sync_translations')}</h3>
                                                <p className="text-[10px] text-[var(--text-muted)] leading-relaxed italic">{t('sync_desc_manual')}</p>
                                            </div>
                                            <button
                                                onClick={refreshTranslations}
                                                disabled={syncingI18n}
                                                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-600/50 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-indigo-600/20"
                                            >
                                                {syncingI18n ? t('syncing') : t('sync_now')}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <section className="space-y-6">
                            <div className="flex items-center gap-3 border-b border-[var(--border-color)] pb-4">
                                <span className="material-symbols-outlined text-rose-400 text-3xl">api</span>
                                <div>
                                    <h2 className="text-xl font-bold font-display">{t('erp_integration_title')}</h2>
                                    <p className="text-sm text-[var(--text-muted)]">{t('erp_integration_desc')}</p>
                                </div>
                            </div>

                            <div className="bg-[var(--bg-card)] p-6 rounded-[2rem] border border-[var(--border-color)] space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">{t('erp_endpoint')}</label>
                                        <input
                                            type="text"
                                            placeholder="https://api.tu-erp.com/v1/data"
                                            className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl p-3 text-sm focus:border-indigo-500 outline-none transition-all text-[var(--text-main)]"
                                            value={settings.erp_endpoint || ''}
                                            onChange={(e) => setSettings({ ...settings, erp_endpoint: e.target.value })}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">{t('erp_api_key')}</label>
                                        <input
                                            type="password"
                                            placeholder="••••••••••••••••"
                                            className="w-full bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl p-3 text-sm focus:border-indigo-500 outline-none transition-all text-[var(--text-main)]"
                                            value={settings.erp_api_key || ''}
                                            onChange={(e) => setSettings({ ...settings, erp_api_key: e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="p-4 bg-amber-500/5 rounded-2xl border border-amber-500/10 flex items-start gap-4">
                                    ...
                                    <p className="text-[10px] text-amber-500/80 font-medium leading-relaxed italic">
                                        Para que la migración funcione, el endpoint debe devolver un JSON con campos que coincidan con los de las tablas de APS (id, name, quantity, etc.) o configurar el mapeo avanzado.
                                    </p>
                                </div>
                            </div>
                        </section>
