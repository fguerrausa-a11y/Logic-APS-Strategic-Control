...
export const sendMessageToGemini = async (message: string, extraContext?: any, language: string = 'es'): Promise<string> => {
  ...
  for (const modelName of modelsToTry) {
    try {
      const response = await generateWithModel(modelName, message, extraContext, language);
      return response;
    } catch (error: any) {
  ...
  try {
    ...
    const response = await generateWithFetch("gemini-2.0-flash", message, extraContext, language);
    return response;
  } catch (error: any) {
...
const generateWithModel = async (modelName: string, promptText: string, extraContext?: any, language: string = 'es'): Promise<string> => {
  const API_KEY = import.meta.env.VITE_GEMINI_API_KEY || "";
  ...
  const systemPrompt = `
    ...
    5. IDIOMA: Responde SIEMPRE en el idioma asociado al código: [${language}]. Es CRÍTICO que toda la respuesta esté en este idioma.
...
const generateWithFetch = async (modelName: string, promptText: string, extraContext?: any, language: string = 'es'): Promise<string> => {
  const API_KEY = import.meta.env.VITE_GEMINI_API_KEY || "";
